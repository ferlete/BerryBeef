        ...
        try:
            ...
            if file.file_exists(config_file):
                ...
                if trm_name:
                    ...
                    ph_model = DB_FOLDER + config.get("Model", "ph")
                    ...
                    if data_predict.shape[0] != data_model_ph.shape[1] - 1:
                        ...
                        reference = DB_FOLDER + config.get("Reference", "ref")
                        ...
                        for key, value in config.items('Model'):
                            key_model = DB_FOLDER + config.get("Model", key)
                            key_parameter = config.get("Parameter", key)
                            ...
                            if round(float(Y_pred[0]), 3) > float(key_parameter.replace(',', '.')):
                                classifier = "A"
                            else:
                                classifier = "R"

                            out = out + "{} = {} {}\n".format(key, round(float(Y_pred[0]), 3), classifier)
